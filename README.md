...
3. 사용자마다 자신의 **사진을 기반으로 한 고정 형태의 가상 인플루언서**를 생성하고, 해당 인플루언서로 **이미지와 텍스트를 생성하여 SNS 에 피드**를 남기는 서비스를 구상.  
   => 인플루언서의 얼굴/체형/헤어스타일 등을 고정하여 이미지를 생성하는 기능이 유료모델에서만 가능하다는 점때문에 <span style='color: red;'>폐기</span>
4. 가상 인플루언서의 **얼굴/체형/헤어스타일을 고정할 수 있는 유료 모델**을 쓰되, 사용량 예측이 어려운 일반사용자들 대신,  
   단일 가상 인플루언서만을 생성하고, 해당 **인플루언서가 활동하는 모습에 대한 이미지와 텍스트를 생성하고 SNS 에 피드**를 남기는 서비스를 구상.  
   => 주어진 주제에 비해 RAG나 LLM 등 실제 개발해보거나 참여해볼 수 있는 부분이 너무 적다는 멘토링 피드백으로 <span style='color: red;'>폐기</span>
5. **친환경 제품들을 판매하는 브랜드를 알릴 수 있는 정보전달용 사이트**를 구축하고, 해당 사이트에서 **인공지능 인플루언서가 TTS 로 정보를 전달하는 Webapp** 구상.  
   => '친환경 제품' 이라는 범주가 너무 크고, 프로젝트의 주제와 방향을 알기 어렵다는 피드백에 따라 <span style='color: red;'>기획 수정</span>
6. <span style='color: green; font-size:bold; font-size:16px;'>최종 주제</span>  
